  ...
  try {
    ...
    if (error.status === 429 || error.message?.includes("quota") || error.message?.includes("429")) {
      return res.status(429).json({ 
        ...
        message: "Senhor, atingimos o limite de requisições gratuitas do Google. Por favor, aguarde um minuto antes de tentar novamente ou verifique sua cota no Google AI Studio." 
      });
    }
